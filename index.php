...
$populer_yazilar = $baglanti->query("SELECT id, baslik, goruntulenme, slug FROM posts WHERE durum = 'yayinda' ORDER BY goruntulenme DESC LIMIT 5");
        ...
        <div class="carousel-inner">
            ...
            while ($s = $slider_yazilar->fetch_assoc()): ?>
                <div class="carousel-item <?= $aktif ? 'active' : '' ?>">
                    <?php $aktif = false; ?>
                    <img src="<?= "./admin/" . $s["gorsel"] ?>" class="d-block w-100" style="object-fit: cover; height: 350px;" alt="">
                    <div class="carousel-caption d-none d-md-block bg-dark bg-opacity-50 p-2 rounded">
                        <h5><?= htmlspecialchars($s["baslik"]) ?></h5>
                        <p><?= mb_strimwidth(strip_tags($s["icerik"]), 0, 120, "...") ?></p>
                        <a href="post.php?id=<?= $s["id"] ?>" class="btn btn-sm btn-light">Devamını Oku</a>
                    </div>
                </div>
            <?php endwhile; ?>
        </div>
